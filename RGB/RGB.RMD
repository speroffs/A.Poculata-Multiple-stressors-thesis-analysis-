...
# Re-plot figure 8, but reverse the Y axis 
```{r}
fig8.rev<-
ggplot(data = treat.sum.test,
              aes(x = Time, 
...
```
```{r}
ggsave(plot = fig8.rev, 
       filename = "/Users/speroffs/Desktop/fig8.rev.jpg", 
       width = 8, height = 6, units = "in", dpi = 300)
```
...
```{r}
# Summarize to plot
phen.sum1.1 <- summarySE(exp.data, measurevar = "CoralR", groupvars = "Sym.State")
...
ggplot(data = plastic.sum4.4,
 ...
 # scale_y_continuous(name = "", limits = c(0.39, 0.55), breaks = seq(0.4, 0.55, by = 0.05)) + # set y axis intervals
  scale_y_reverse(name = "", limits = c(0.55,0.39), breaks = seq(0.55, 0.4, by = -0.05)) +
  ggtitle ("A. Plastic") +
  
  facet_grid(~Plastic, labeller = plastic_labeller) +
  theme_classic(base_size=13) +
  theme(legend.position = "none", #c(0.7,1.3)
        legend.title = element_blank(),
...
```{r}
rgb.panel1 <- ggarrange(rbg.plastic.plot, ggplot()+theme_void(), 
                        widths = c(2, 1), common.legend = TRUE, legend = "right")

rgb.panel2 <- ggarrange(rbg.light.plot, rbg.food.plot,
                        common.legend = TRUE, 
                        labels = NA, legend = "none",
                        widths=c(1,1))

panel <- ggarrange(rgb.panel1, rgb.panel2,
                   common.legend = FALSE,
                   labels = NA,
                   ncol=1)

rgb.panel1 <-annotate_figure(panel, bottom = text_grob("Day", 
                            hjust = 0.4, vjust = -0.5, size = 18), 
                            #face = "bold"),
                            left = text_grob("Intensity in the red channel", 
                                 rot = 90, vjust = 1, hjust = 0.4, size = 18))
                                 #face = "bold")) 

ggsave(rgb.panel1, 
       filename = "/Users/speroffs/Desktop/rgb.panel.stressors.jpg",
       width = 8, height = 6, units = "in", dpi = 300)
```
...
ggplot(amb.ctrl.data, 
  ...
  geom_point(aes(shape=Sym.State, 
                 fill=Sym.State), # for presentation
             size = 3, 
             alpha = 0.9) +
  ...
  scale_shape_manual(labels = c("Aposymbiotic", "Symbiotic"), values=c(21,21)) +
  scale_color_manual(labels = c("Aposymbiotic", "Symbiotic"), values = c("black", "black")) +
  scale_fill_manual(labels = c("Aposymbiotic", "Symbiotic"), values=c("tan","coral4")) +

  theme_classic(base_size = 13) +
  theme(legend.position = "top",
        legend.title = element_blank(),
        legend.text = element_text(size = 12)) +
    annotate("rect", xmin=0.01, xmax = 0.21, ymin = 0.01, ymax = 0.06, color = "white", fill = "white")


ctrl.LR.plot<-annotate_figure(ctrl.LR.plot, 
                              bottom = text_grob("y = 0.809x - 0.0386", 
                              hjust = 1.8, 
                              # vjust = -10,
                              vjust = -9.2,
                              size = 12)) 
...
ggsave(plot = ctrl.LR.plot, 
       filename = "/Users/speroffs/Desktop/ctrl.LR.plot.jpg", 
       width = 8, height = 6, units = "in", dpi=300)
```
