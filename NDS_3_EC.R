...
#
#       Date: 10/20/2021
#       Experiment name NDS_3_10.20.21_e.coli:
...
# data import
data <- read.csv("NDS_3_10.20.21_e.coli.xlsx - formated.csv", header=TRUE)
str(data)
...
MO2_A2 <- convert_rate(A2_adjusted_rate,
                       ...
                       t = 19.1,                 # temperature
                       S = 35,                   # salinity 
                       P = 1012.5/1013)               # atmospheric pressure
...
# Calculate mass-specific rate 
MO2_D3<- convert_rate(D3_adjusted_rate,
                      o2.unit = "%",            # original O2 units
                      time.unit = "min",        # time units 
                      output.unit = "umol/min/g",     # desired rate units 
                      volume = 1.9,             # chamber liquid volume
...
MO2_df$B.TAG <- (c("G37", "B34", "B36", "W29",  "W31",                  
                   "Y25", "Y27", "Y29",
                   "G75", "G77", "B76", "B80", "W73", 
                   "W75", "Y67", "Y71", "Y73"))
# Add "day"
...
# add sym state
MO2_df$Sym.state <- c(rep("sym", times = 8), rep("apo", times = 9))
...
#write.csv(MO2_df, file.path(path, "NDS_3_EC_allrate.csv"))
