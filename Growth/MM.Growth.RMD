...
# We'll continue from here working with the Log transformed data:
```{r}
# summarize the mass at each interval by treatment 
decay_means <- summarySE(decaydat, measurevar = "Logmassmg", groupvars = c("Treatment", "Day"))
```
```{r}
# plot 
ggplot(data = decay_means, 
  ...
  geom_errorbar(aes(ymax = Logmassmg+se, ymin = Logmassmg-se), width = 0.1) +
  ylab("Log Mass (mg)")+
  xlab("Day") +
...
```{r}
# summarize the mass at each interval by treatment 
decay_means2 <- summarySE(decaydat, measurevar = "Logmassmg", groupvars = c("Treatment", "Day", "Sym.State"))
```
```{r}
# plot 
...
ggplot(data = decay_means, 
  ...
  geom_errorbar(aes(ymax = Logmassmg+se, ymin = Logmassmg-se),width =1, size=1.2) +
...
treat.lmer <- lmer(Slope~Treatment + (1|Colony), data = ID.slope)
summary(treat.lmer)
```
```{r}
# Run an anova to compare this model to a null model 
null <- lmer(Slope ~ 1 + (1|Colony), data = ID.slope)
...
ggplot(data = treat.means,
  ...
  geom_hline(yintercept = -0.003744175, col="black", lty=2) + 
  geom_hline(yintercept = 0, col="black", lty=1) +
  
  scale_y_continuous(name = "Average growth rate [log(mg*day"^"-1"~")]", limits = c(-0.016, 0), breaks = seq(-0.016, 0, by = 0.004)) + # set y axis intervals
...
```{r}
# save plot
ggsave(plot = all.treat.log.growth.plot,
       filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/Growth/Plots/all.treat.log.growth.plot.pdf", width = 8, height = 6, units = "in")
...
```{r}
# Run a liner effects mixed model to determine if the treatment affects the growth rate of polyps while taking coral colony into account
temp.lmer <- lmer(Slope~Temperature + (1|Colony), data = ID.slope)
summary(temp.lmer)
```
```{r}
# Run an anova to compare this model to a null model 
...
plastic.lmer <- lmer(Slope~Plastic + (1|Colony), data = ID.slope)
...
light.lmer <- lmer(Slope~Light + (1|Colony), data = ID.slope)
summary(light.lmer)
```
```{r}
# Run an anova to compare this model to a null model 
...
food.lmer <- lmer(Slope~Food + (1|Colony), data = ID.slope)
summary(food.lmer)
```
```{r}
# Run an anova to compare this model to a null model 
...
phen.lmer <- lmer(Slope~Sym.State + (1|Colony), data = ID.slope)
summary(phen.lmer)
```
```{r}
# Run an anova to compare this model to a null model 
...
# Consider phenotype
```{r}
m3 <- lmer(Slope ~ Temperature + Plastic + Light + Food + Sym.State + (1|Colony), data = ID.slope)
summary(m3)
```
```{r}
# drop plastic
m4 <- lmer(Slope ~ Temperature + Light + Food + Sym.State + (1|Colony), data = ID.slope)
...
```{r}
m5 <- lmer(Slope ~ Temperature + Light + Food + Light*Food + (1|Colony), data = ID.slope)
summary(m5)
```
```{r}
...
```
```{r}
m <- lmer(Slope ~ Temperature + Light + Food + (1|Colony), data = ID.slope)
```
```{r}
# plot light
...
int.m<-lmer(Slope ~ Temperature + Light + Food + (1|Colony), data = ID.slope)
summary(int.m)
