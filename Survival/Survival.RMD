...
```{r exp data}
exp.data <- filter(surv.dat, Treatment != "amb.ctrl")
summary(exp.data) # 42 per treat, 168 per stress
exp.graph.data <- filter(graph.dat, Treatment != "amb.ctrl")
summary(exp.graph.data) # 40 indiv per treat, 160 per stress
...
plastic1 <- survfit(Surv(Time, Death) ~ Plastic, data = exp.graph.data)
...
## Food in experimental conditions
```{r}
# CPH model
...
```{r exp.food km}
# plot graph data (also subsetted for elevated temp)
food1 <- survfit(Surv(Time, Death) ~ Food, data = exp.graph.data)
...
## Light in experimental conditions
```{r}
# CPH model
...
```{r exp.light km}
# plot graph data (also subsetted for elevated temp)
light1 <- survfit(Surv(Time, Death) ~ Light, data = exp.graph.data)
...
km.temp.fig <- 
  ggsurvplot(temperature, 
          conf.int = FALSE, pval = FALSE, surv.scale = "default", # or %
          legend.labs = c("Ambient (19 C)", "Elevated (30 C)"), legend.title = "", legend = c(0.3, 0.2), font.legend = 19,
          xlab = "", xlim = c(0, 23),break.x.by = 2, font.x = 14,
          ylab = "", ylim = c(0.75,1),font.y= 14,
          font.tickslab = 14,
          censor.size = 6,
...
```{r phen.light km}
# plot graph data (also subsetted for elevated temp)
phen1 <- survfit(Surv(Time, Death) ~ Sym.State, data = exp.graph.data)
