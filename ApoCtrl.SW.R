...
#
#       Date: 10/28/2021
#       Experiment name: Apo_Control_10.28.21_sw
...
# data import
data <- read.csv("Apo_Control_10.28.21_sw.xlsx - formated.csv", header=TRUE)
str(data)
...
MO2_A2 <- convert_rate(A2_adjusted_rate,
                       ...
                       mass = 0.098,              # mass in GRAMS
                       t = 23.5,                 # temperature
                       S = 35,                   # salinity 
                       P = 1015.7/1013)               # atmospheric pressure
...
MO2_D6<- convert_rate(D6_adjusted_rate,
                      ...
                      time.unit = "min",        # time units 
                      output.unit = "umol/min/g",     # desired rate units 
                      volume = 1.7,             # chamber liquid volume
                      mass = 0.177,              # mass in GRAMS
                     t = 23.5,                 # temperature
                      S = 35,                   # salinity 
                      P = 1015.7/1013)               # atmospheric pressure
...
MO2_df$B.TAG <- (c("O78", "O80", "B6334", "O85", "O87",
                   "G6961", "O89", "O93", "G9669", "O98",
                   "Y113", "G8171", "Y164", "G6991", "Y1819",
                   "O916", "Y3225", "O6968", "Y4451", "W7"))
## Add "day"
MO2_df$Day <- "22"

# add sym state
MO2_df$Sym.state <- "apo"
...
# export
#write.csv(MO2_df, "Apo_Ctrl_3_SW_allrate.csv")
...
apo.list <- list(A2.data, A3.data, A4.data, A5.data, A6.data,
...
write.csv(apo.dat, "Apo_Ctrl_SW_rate.csv")

#plot
ggplot(apo.dat, aes(Time.Elapsed.Min, value, col = vial)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method="lm", se=FALSE) +
  ggtitle("Apo Control 3 SW") +
...
# Visualize
g1 <- ggplot(apo.dat, aes(Time.Elapsed.Min, value, col = vial)) +
  geom_point(alpha = 0.3, show.legend = FALSE) +
  geom_smooth(method="lm", se=FALSE, show.legend = FALSE) +
  scale_x_continuous(limits = c(0, 240)) +
  scale_y_continuous(limits = c(60, 100)) +
  ggtitle("Apo Control, T3, Saltwater") +
...
g2 <- ggplot(apo.dat, aes(Time.Elapsed.Min, value, col = sym.state)) +
