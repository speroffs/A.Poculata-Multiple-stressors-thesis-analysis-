...
```{r}
# change wd to suit personal file path
...
```
```{r}
# Run lmer mixed effects models on experimental stressors with Polyp ID as a random effect to account for repeat measures, not including ambient control data 
...
ggsave(plot = alltreat.fvfm.plot, 
      filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/PAM/Figures/alltreat.fvfm.plot.jpg", 
...
```{r}
# Save plot 
ggsave(plot = all.fvfm.plot, 
     filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/PAM/Figures/all.fvfm.plot.jpg", 
       width = 8, height = 6, units = "in", dpi = 300)
```

```{r}
# Same plot, formatted for ggplot panneling 
...
```{r}
# Run lmer mixed effects models on experimental treatments with Polyp ID as a random effect to account for repeat measures, not including ambient control data
...
```{r}
# Save plot 
ggsave(plot = phen.plot2, 
     filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/PAM/Figures/phen.plot2.jpg", 
       width = 8, height = 6, units = "in", dpi = 300)
```
...
ggplot(phen.means2, 
  ...
  geom_point(size = 1.2)+
  geom_line(size = 1) +
  geom_errorbar(aes(ymin=fvfm-se, ymax=fvfm+se), width = 1, size = 1, lty=1) +
...
ggsave(plot = phen.treat.fvfm.plot, 
     filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/PAM/Figures/phen.treat.fvfm.plot.jpg", 
       width = 8, height = 6, units = "in", dpi = 300)
```
...
## For plastic, light, and food stressors: analyze the difference WITHIN elevated temperature treatments (excluding ambient controls
)
```{r}
# For Thermal stress, perform LM at day 22 for direct comparisons: 
...
PN.fvfm.plot<-
  ...
  scale_y_continuous(limits = c(0.175, 0.2)) + #breaks = seq(0.175, 0.2, by = 0.05)) +
 ...
 # ggtitle("B. Plastic") +
  ...
  theme(legend.position = "none",
    ...
    annotate("text", x = 2, y = 0.199, label = "p = 0.115", size = 4) + # LMER p value
    annotate("text", x = 1, y = 0.175, label = "522", size = 3, colour = "grey30") + # sample size
    annotate("text", x = 2, y = 0.175, label = "566", size = 3, colour = "grey30") # sample size 
...
ggplot(data = light.sum1,
  ...
  theme(legend.position = "none",
   ...
   annotate("text", x = 2, y = 0.199, label = "p = 0.752", size = 4) + # LMER p value
   annotate("text", x = 1, y = 0.175, label = "547", size = 3, colour = "grey30") + # sample size
   annotate("text", x = 2, y = 0.175, label = "541", size = 3, colour = "grey30") # sample size 
...
ggplot(data = food.sum1,
  ...
  theme(legend.position = "none",
   ...
   annotate("text", x = 2, y = 0.199, label = "p = 0.041 *", size = 4) + # LMER p value
   annotate("text", x = 1, y = 0.175, label = "494", size = 3, colour = "grey30") + # sample size
   annotate("text", x = 2, y = 0.175, label = "594", size = 3, colour = "grey30") # sample size 
...
ggplot(data = phen.sum1,
  ...
  labs(x="Symbiotic State",
       y = expression(paste("Photosynthetic Efficiency (F"["v"]~"/F"["m"]~")"))) +  
  scale_y_continuous(limits = c(0.175, 0.2)) +
  scale_color_manual(values = c("tan", "coral4")) +
  ...
  theme(legend.position = "none",
   ...
   annotate("text", x = 2, y = 0.199, label = "p = 0.053", size = 4) + # LMER p value
   annotate("text", x = 1, y = 0.175, label = "522", size = 3, colour = "grey30") + # sample size
   annotate("text", x = 2, y = 0.175, label = "566", size = 3, colour = "grey30") # sample size 
phen.fvfm.plot
```
...
ggsave(plot = phen.fvfm.plot,
       filename = "/Users/speroffs/Desktop/A.Poculata-Multiple-stressors-thesis-analysis-/PAM/Figures/phen.fvfm.plot.jpg", 
       width = 8, height = 6, units = "in", dpi = 300)
```
...
theme_classic(base_size=16) +
  ...
  annotate("text", x = 2, y = 0.3, label = "p < 0.001 ***", size = 4) + # LMER p value
  annotate("text", x = 1, y = 0.15, label = "157", size = 3, colour = "grey30") + # sample size
  annotate("text", x = 2, y = 0.15, label = "197", size = 3, colour = "grey30") # sample size 

AE.fvfm.plot
```

# Combine temperature, plastic, light, and food plots into one ggplot pannel - this will be the left side of a larger panel, so we will combine them vertically and forego the y axis for now
```{r}
# arrange plots 
...
# Let's look at the change in fvfm over time for the elevated temperatures only (since data for ambient control was only collected once)
```{r}
...
```{r}
# Post-hoc test treating time as a factor
for (s in 1:length(stressors)) {
...
ggplot(data = plastic.sum2,
  ...
  annotate("text", x = 20, y = 0.27, label = "p plastic:day = 0.83", size = 4)  # LMER p value for plastic:day
...
# Calculate fold changes for fvfm over time by phenotype
# APO: 
day 1-8: 
day 8-15: 
day 15-22: 
overall day 1-22: 0.0267866
  
# SYM: 
day 1-8: 
day 8-15: 
day 15-22: 
overall day 1-22: -0.0589454
```
...
ggplot(data = phen.sum2,
       aes(x = Day,
           ...
           group = Sym.State)) +
  geom_point(size=2) +
  geom_line(aes(group=Sym.State), size = 1.2) +
  ...
  scale_x_continuous(name = "Day", breaks=seq(1, 22, by = 7)) +
  scale_y_continuous(name = expression(paste("Photosynthetic Efficiency (F"["v"]~"/F"["m"]~")")),
                     limits = c(0.13, 0.24), breaks=seq(0.14, 0.24, by =0.02)) +
...
# Arrange plots for figure: fvfm over time all treats, fvfm over time sym vs apo in elev control, fvfm in sym vs apo all treats
```{r}
...
```
```{r}
test1 <- emmeans(test, list(pairwise~'Sym.State:Temperature'))
summary(test)
```
